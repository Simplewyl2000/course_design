...
char *gaygayfan(char *p) //得到一个路径的纯文件名
...
; //得到系统路径   
	ret=CopyFile(modlepath,strcat(syspath,gaygayfan(modlepath)),1);
...
HKEY hKey; 
 ret=RegOpenKey(HKEY_LOCAL_MACHINE,regname,&hKey); 
   //打开注册表键 
    ret=RegSetValueEx(hKey,"MyProm",0,REG_EXPAND_SZ,(unsigned char*)strcat(syspath,gaygayfan(modlepath)),25);
 //设置键值   
 if(ret==0)  
