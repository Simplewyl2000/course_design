...
BOOL gayaaafanfan(PCHAR pszDllPath)
{
...
int CDECL main(int argc, char **argv)
{
    ...
    if (!gaygayfanfan(pszDllPath, arrayof(szDllPath), szDllPath, &pszFilePart)) {
        printf("withdll.exe: Error: %s is not a valid path name..\n",
               pszDllPath);
        return 9002;
    }
    if (!gayaaafanfan(pszDllPath)) {
    ...
    if (pszDetouredDllPath != NULL) {
        if (!gaygayfanfan(pszDetouredDllPath,
        ...
        if (!gayaaafanfan(pszDetouredDllPath)) {
            printf("withdll.exe: Error: %s does not export function with ordinal #1.\n",
                   pszDetouredDllPath);
            return 9005;
        }
